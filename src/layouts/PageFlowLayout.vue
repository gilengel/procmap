<template>
  <q-layout>
    ...
    <div class="row">
      <div class="col">
        <q-btn v-if="previousConnection.incoming_id" :label="previous" color="primary" @click="gotoAnotherPage(previousConnection.incoming_id)" />
      </div>
      <div class="col">
      </div>
      <div class="col">
        <q-btn v-if="follwingConnection.outgoing_id !== $route.params.id" :label="next" color="primary" @click="gotoAnotherPage(follwingConnection.outgoing_id)" />
      </div>
    </div>
...
export default class PageFlowLayout extends Vue {
  ...
  private getConnectionsFromBackend() {
    ...
    GetOne<Page>(`${PAGES_URL}/current/${currentPageId}`).then(
      page => {
    ...
    GetMultiple<RouterPair>(`pages/following/${this.$route.params.id}`).then(
      (pairs) => {
        if(pairs.length > 0) {
          this.follwingConnection = Object.assign({}, this.follwingConnection, pairs[0]);
        }
      }
    );
    GetMultiple<RouterPair>(`pages/previous/${this.$route.params.id}`).then(
      (pairs) => {
        if(pairs.length > 0) {
          this.previousConnection = Object.assign({}, this.previousConnection, pairs[0]);
        }
      }
    );
  }
